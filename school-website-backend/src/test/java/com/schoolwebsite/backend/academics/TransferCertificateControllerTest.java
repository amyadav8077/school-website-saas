...
public class TransferCertificateControllerTest {
    ...
    public void testTCLookupAndSearch() {
        // Create Tenant
        Tenant tenant = Tenant.builder().name("Springfield High").subdomain("sfhigh").status("ACTIVE").build();
        Tenant savedTenant = tenantRepository.save(tenant);
        Long tenantId = savedTenant.getId();

        // Create and save Transfer Certificate
        TransferCertificate tc = TransferCertificate.builder().tenantId(tenantId).studentName("Lisa Simpson")
                .admissionNo("ADM-456").classLevel("8th").section("A").fatherName("Homer Simpson")
                .aadharNo("4444-5555-6666").tcNumber("TC-2026-88").issueDate(LocalDateTime.now()).build();
        ...
        List<TransferCertificate> classList = repository
                .findByTenantIdAndClassLevelAndSectionOrderByIssueDateDesc(tenantId, "8th", "A");
        assertFalse(classList.isEmpty());
        assertEquals("Lisa Simpson", classList.get(0).getStudentName());

        // Test Name containing Lookup
        List<TransferCertificate> nameList = repository
                .findByTenantIdAndStudentNameContainingIgnoreCaseOrderByIssueDateDesc(tenantId, "lisa");
