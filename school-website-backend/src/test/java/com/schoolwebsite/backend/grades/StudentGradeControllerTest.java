...
public class StudentGradeControllerTest {
    ...
    public void testGradesDemographicsAndLookups() {
        // Create Tenant
        Tenant tenant = Tenant.builder().name("Springfield School").subdomain("springfield").status("ACTIVE").build();
        Tenant savedTenant = tenantRepository.save(tenant);
        Long tenantId = savedTenant.getId();

        // Save a Student Grade record with demographics
        StudentGrade grade = StudentGrade.builder().tenantId(tenantId).studentName("Bart Simpson")
                .subjectName("Mathematics").term("Term 1 Midterm").grade("C-")
                .remarks("Needs to focus more on home assignments.").admissionNo("ADM-201").classLevel("4th")
                .section("B").fatherName("Homer Simpson").aadharNo("1111-2222-3333").build();
        ...
        List<StudentGrade> classList = repository.findByTenantIdAndClassLevelAndSectionOrderByCreatedAtDesc(tenantId,
                "4th", "B");
        assertFalse(classList.isEmpty());
        assertEquals("Bart Simpson", classList.get(0).getStudentName());

        // Test name-wise containing lookup
        List<StudentGrade> nameList = repository
                .findByTenantIdAndStudentNameContainingIgnoreCaseOrderByCreatedAtDesc(tenantId, "Bart");
