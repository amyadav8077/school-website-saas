    ...
    public void run(String... args) throws Exception
    {
        if (adminUserRepository.findByUsername("admin").isEmpty())
        {
            adminUserRepository.save(
                    AdminUser.builder().username("admin").password(passwordEncoder.encode("admin123"))
        ...
        if (forceRefresh && tenantRepository.existsBySubdomain("pioneer"))
        {
            ...
            System.out.println(
                    "[DatabaseSeeder] SEED_FORCE_REFRESH is enabled — " + "deleting existing 'pioneer' tenant (id="
                            + oldId + ") and all cascaded data for a full reseed.");
        ...
        if (!tenantRepository.existsBySubdomain("pioneer"))
        {
            ...
            Tenant tenant = tenantRepository.save(
                    Tenant.builder().name("SaaS Pioneer Academy").subdomain("pioneer").status("ACTIVE").build());
            Long tid = tenant.getId();

            adminUserRepository.save(
                    AdminUser.builder().username("pioneer_admin").password(passwordEncoder.encode("pioneer123"))
                            .role("TENANT_ADMIN").tenantId(tid).email("admin@pioneer.edu").phoneNumber("+914010233440")
                            .build());

            // ── SITE CONFIG ───────────────────────────────────────────────
            siteConfigRepository.save(
                    SiteConfig.builder().tenantId(tid).logoUrl("🎓").themeName("ROYAL_NAVY").primaryColor("#1e3a8a")
                            .secondaryColor("#991b1b").accentColor("#fbbf24").fontFamily("Inter")
                            .contactEmail("admissions@pioneer.edu").contactPhone("+91 40 1023 3440").socialLinks(
            ...
            Page home = pageRepository.save(
                    Page.builder().tenantId(tid).title("Home").slug("home").status("PUBLISHED").build());
            pageSectionRepository.save(PageSection.builder().pageId(home.getId()).type("HERO").positionOrder(1)
                    .config("{\"title\":\"Nurturing Potential, Inspiring Excellence\",\"subtitle\":\"Welcome to SaaS Pioneer Academy — where high-tech labs, championship athletics, and world-class mentorship shape tomorrow's leaders.\"}")
                    .build());
            ...
            pageSectionRepository.save(PageSection.builder().pageId(home.getId()).type("VIDEO").positionOrder(7)
                    .config("{\"title\":\"Pioneer Academy — Virtual Campus Tour\",\"video_url\":\"https://www.youtube.com/embed/dQw4w9WgXcQ\"}")
                    .build());
            ...
            pageSectionRepository.save(PageSection.builder().pageId(adm.getId()).type("HERO").positionOrder(1)
                    .config("{\"title\":\"Secure Your Child's Academic Future\",\"subtitle\":\"Admissions for the cohort 2026-27 are officially open. Limited seats available — register your interest today.\"}")
                    .build());
            ...
            Page courses = pageRepository.save(
                    Page.builder().tenantId(tid).title("Academics").slug("courses").status("PUBLISHED").build());
            ...
            // FACULTY PAGE
            Page faculty = pageRepository.save(
                    Page.builder().tenantId(tid).title("Faculty").slug("faculty").status("PUBLISHED").build());
            ...
            pageSectionRepository.save(PageSection.builder().pageId(fees.getId()).type("HERO").positionOrder(1)
                    .config("{\"title\":\"Fee Structure & Online Payments\",\"subtitle\":\"Transparent, structured fee schedules with secure online payment options for all grades.\"}")
                    .build());
            ...
            // CONTACT PAGE
            Page contact = pageRepository.save(
                    Page.builder().tenantId(tid).title("Contact Us").slug("contact").status("PUBLISHED").build());
            ...
            // NEWS & EVENTS PAGE
            Page news = pageRepository.save(
                    Page.builder().tenantId(tid).title("News & Events").slug("news").status("PUBLISHED").build());
            pageSectionRepository.save(PageSection.builder().pageId(news.getId()).type("HERO").positionOrder(1)
                    .config("{\"title\":\"Latest News & Upcoming Events\",\"subtitle\":\"Stay updated with school announcements, circulars, event schedules, and achievement highlights.\"}")
                    .build());
            ...
            // CAREERS PAGE
            Page careers = pageRepository.save(
                    Page.builder().tenantId(tid).title("Careers").slug("careers").status("PUBLISHED").build());
            ...
            // DISCLOSURES PAGE
            Page disclosures = pageRepository.save(
                    Page.builder().tenantId(tid).title("Mandatory Disclosures").slug("disclosures").status("PUBLISHED")
                            .build());
            pageSectionRepository.save(PageSection.builder().pageId(disclosures.getId()).type("HERO").positionOrder(1)
                    ...
                    .build());
            pageSectionRepository.save(
                    PageSection.builder().pageId(disclosures.getId()).type("DISCLOSURES").positionOrder(2)
                            ...
                            .build());
            pageSectionRepository.save(
                    PageSection.builder().pageId(disclosures.getId()).type("NOTICES").positionOrder(3)
            ...
            pageSectionRepository.save(PageSection.builder().pageId(tc.getId()).type("HERO").positionOrder(1)
                    .config("{\"title\":\"Transfer Certificate Verification\",\"subtitle\":\"Parents and students can verify and download issued Transfer Certificates online using admission number and personal details.\"}")
                    .build());
            pageSectionRepository.save(PageSection.builder().pageId(tc.getId()).type("NOTICES").positionOrder(2)
                    .config("{\"title\":\"TC Application Guidelines\",\"notices\":[{\"text\":\"TC requests take 5-7 working days to process\"},{\"text\":\"Original fee receipts must be cleared before TC issuance\"},{\"text\":\"Apply in person at the admin office with parent's ID proof\"},{\"text\":\"For urgent TC: contact admin@pioneer.edu\"}]}")
                    .build());

            // ACHIEVEMENTS PAGE
            Page achievements = pageRepository.save(
                    Page.builder().tenantId(tid).title("Achievements").slug("achievements").status("PUBLISHED")
                            .build());
            pageSectionRepository.save(PageSection.builder().pageId(achievements.getId()).type("HERO").positionOrder(1)
                    ...
                    .build());
            pageSectionRepository.save(
                    PageSection.builder().pageId(achievements.getId()).type("CAROUSEL").positionOrder(2)
            ...
            courseRepository.save(AcademicCourse.builder().tenantId(tid).name("Biology & Life Sciences (NEET)")
                    .gradeLevel("High School (G11-12)").description(
            ...
            courseRepository.save(AcademicCourse.builder().tenantId(tid).name("Social Studies & Civics")
                    .gradeLevel("Middle School (G6-8)").description(
            ...
            courseRepository.save(
                    AcademicCourse.builder().tenantId(tid).name("Robotics & AI Fundamentals").gradeLevel("All Grades")
                            .description(
                                    "Hands-on coding, microcontroller programming, and introduction to artificial intelligence.")
                            .syllabusSummary("Python, Arduino, Scratch, Machine Learning Basics, Robot Design")
                            .build());
            ...
            programRepository.save(
                    AcademicProgram.builder().tenantId(tid).name("Integrated Junior Colleges (11-12)").type("COLLEGE")
                            .description(
                            ...
                            .build());
            programRepository.save(
                    AcademicProgram.builder().tenantId(tid).name("NEET & JEE Premier Coaching").type("COACHING")
                            .description(
                                    "India's most structured engineering and medical entrance rank training program.")
                            .details(
            ...
            achieverRepository.save(
                    StudentAchiever.builder().tenantId(tid).name("Riddhi Sharma").score("CBSE Class 12 — 499/500")
                            .courseName("Commerce Stream").testimonialText(
                            ...
                            .build());
            achieverRepository.save(
                    StudentAchiever.builder().tenantId(tid).name("Somya Reddy").score("NEET UG — AIR 14")
                            .courseName("Pre-Medical Coaching").testimonialText(
                            ...
                            .build());
            achieverRepository.save(
                    StudentAchiever.builder().tenantId(tid).name("B. Sanjana Krishnan").score("CBSE Class 12 — 498/500")
                            .courseName("Science (PCM) Stream").testimonialText(
                            ...
                            .build());
            achieverRepository.save(
                    StudentAchiever.builder().tenantId(tid).name("Arjun Mehta").score("IIT-JEE Advanced — AIR 47")
                            .courseName("IIT-JEE Integrated Program").testimonialText(
                            ...
                            .build());
            achieverRepository.save(
                    StudentAchiever.builder().tenantId(tid).name("Priya Nambiar").score("CBSE Class 10 — 500/500")
                            .courseName("Secondary School (Grade 10)").testimonialText(
            ...
            achieverRepository.save(StudentAchiever.builder().tenantId(tid).name("Karthik Suresh")
                    .score("National Science Olympiad — Gold").courseName("STEM & Olympiad Track").testimonialText(
                    ...
                    .build());
            achieverRepository.save(
                    StudentAchiever.builder().tenantId(tid).name("Meera Iyer").score("CBSE Class 12 — 497/500")
                            .courseName("Humanities Stream").testimonialText(
                            ...
                            .build());
            achieverRepository.save(
                    StudentAchiever.builder().tenantId(tid).name("Aditya Verma").score("IIT-JEE Mains — AIR 122")
                            .courseName("IIT-JEE Integrated Program").testimonialText(
                            ...
                            .build());
            achieverRepository.save(
                    StudentAchiever.builder().tenantId(tid).name("Ishita Gupta").score("NEET UG — AIR 208")
                            .courseName("Pre-Medical Coaching").testimonialText(
            ...
            achieverRepository.save(StudentAchiever.builder().tenantId(tid).name("Rohan Malhotra")
                    .score("International Math Olympiad — Silver").courseName("STEM & Olympiad Track").testimonialText(
                    ...
                    .build());
            achieverRepository.save(
                    StudentAchiever.builder().tenantId(tid).name("Tanvi Joshi").score("CBSE Class 10 — 499/500")
                            .courseName("Secondary School (Grade 10)").testimonialText(
            ...
            galleryRepository.save(
                    GalleryItem.builder().tenantId(tid).type("PHOTO").title("STEM & Robotics Innovation Fair 2025")
                            .category("Academic Expo").mediaUrl(
                                    "https://images.unsplash.com/photo-1526129318478-62ed807ebdf9?auto=format&fit=crop&w=800&q=80")
                            .build());
            galleryRepository.save(
                    GalleryItem.builder().tenantId(tid).type("PHOTO").title("Inter-Branch Soccer League Championship")
                            .category("Sports Day").mediaUrl(
                                    "https://images.unsplash.com/photo-1517649763962-0c623066013b?auto=format&fit=crop&w=800&q=80")
                            .build());
            galleryRepository.save(
                    GalleryItem.builder().tenantId(tid).type("PHOTO").title("Annual Theater & Orchestral Concert")
                            .category("Annual Day").mediaUrl(
                                    "https://images.unsplash.com/photo-1507676184212-d03ab07a01bf?auto=format&fit=crop&w=800&q=80")
                            .build());
            galleryRepository.save(
                    GalleryItem.builder().tenantId(tid).type("VIDEO").title("Morning Assembly & Student Pledge")
                            .category("Campus Life").mediaUrl("https://www.youtube.com/embed/dQw4w9WgXcQ").build());
            galleryRepository.save(
                    GalleryItem.builder().tenantId(tid).type("PHOTO").title("Graduation Ceremony Class of 2025")
                            .category("Annual Day").mediaUrl(
                                    "https://images.unsplash.com/photo-1580582932707-520aed937b7b?auto=format&fit=crop&w=800&q=80")
                            .build());
            galleryRepository.save(
                    GalleryItem.builder().tenantId(tid).type("PHOTO").title("Science Lab — Chemistry Practicals")
                            .category("Academic Expo").mediaUrl(
                                    "https://images.unsplash.com/photo-1567168539593-59673ababaae?auto=format&fit=crop&w=800&q=80")
                            .build());
            galleryRepository.save(
                    GalleryItem.builder().tenantId(tid).type("PHOTO").title("Basketball Inter-House Tournament")
                            .category("Sports Day").mediaUrl(
                                    "https://images.unsplash.com/photo-1546519638-68e109498ffc?auto=format&fit=crop&w=800&q=80")
                            .build());
            galleryRepository.save(
                    GalleryItem.builder().tenantId(tid).type("PHOTO").title("Republic Day Parade & Cultural Program")
                            .category("Cultural Fest").mediaUrl(
                                    "https://images.unsplash.com/photo-1541339907198-e08756dedf3f?auto=format&fit=crop&w=800&q=80")
                            .build());
            galleryRepository.save(
                    GalleryItem.builder().tenantId(tid).type("PHOTO").title("NEET & JEE Topper Felicitation 2025")
                            .category("Academic Expo").mediaUrl(
                                    "https://images.unsplash.com/photo-1509062522246-3755977927d7?auto=format&fit=crop&w=800&q=80")
                            .build());
            galleryRepository.save(
                    GalleryItem.builder().tenantId(tid).type("PHOTO").title("Classical Dance & Music Festival")
                            .category("Cultural Fest").mediaUrl(
                                    "https://images.unsplash.com/photo-1547153760-18fc86324498?auto=format&fit=crop&w=800&q=80")
                            .build());
            galleryRepository.save(
                    GalleryItem.builder().tenantId(tid).type("PHOTO").title("Smart Classroom — Interactive Learning")
                            .category("Campus Life").mediaUrl(
            ...
            branchRepository.save(
                    SchoolBranch.builder().tenantId(tid).name("Madhapur IT-Corridor Campus").state("Telangana")
            ...
            branchRepository.save(
                    SchoolBranch.builder().tenantId(tid).name("Secunderabad Central Campus").state("Telangana")
                            .city("Hyderabad").address("S P Road, Secunderabad, Telangana 500003")
            ...
            branchRepository.save(
                    SchoolBranch.builder().tenantId(tid).name("Dwarka Institutional Campus").state("Delhi")
                            .city("New Delhi").address("Sector 10, Dwarka, New Delhi 110075")
                            .contactEmail("dwarka@pioneer.edu").phone("+91 11 1023 3366").build());
            branchRepository.save(
                    SchoolBranch.builder().tenantId(tid).name("Aundh Knowledge Park Campus").state("Maharashtra")
                            .city("Pune").address("Aundh Road, Pune, Maharashtra 411007")
            ...
            enrichmentRepository.save(EnrichmentActivity.builder().tenantId(tid).type("SPORTS")
                    .title("Sports Athletics & League Championship").description(
            ...
            enrichmentRepository.save(EnrichmentActivity.builder().tenantId(tid).type("UNIFORMS")
                    .title("Official School Uniform Guidelines").description(
            ...
            enrichmentRepository.save(EnrichmentActivity.builder().tenantId(tid).type("EXPO")
                    .title("National STEM & Robotics Innovation Expo").description(
            ...
            enrichmentRepository.save(EnrichmentActivity.builder().tenantId(tid).type("SPORTS")
                    ...
                    .build());
            enrichmentRepository.save(EnrichmentActivity.builder().tenantId(tid).type("EXPO")
                    .title("Annual Cultural & Performing Arts Festival").description(
            ...
            tcRepository.save(
                    TransferCertificate.builder().tenantId(tid).studentName("Harry Potter").admissionNo("ADM-901")
                            .classLevel("10th").section("A").fatherName("James Potter").aadharNo("1234-5678-9012")
                            .tcNumber("TC-2026-001").issueDate(LocalDateTime.now().minusDays(10))
                            .pdfUrl("/tc/TC-2026-001.pdf").build());
            tcRepository.save(
                    TransferCertificate.builder().tenantId(tid).studentName("Ron Weasley").admissionNo("ADM-902")
                            .classLevel("12th").section("B").fatherName("Arthur Weasley").aadharNo("9876-5432-1098")
                            .tcNumber("TC-2026-002").issueDate(LocalDateTime.now().minusDays(5))
                            .pdfUrl("/tc/TC-2026-002.pdf").build());
            tcRepository.save(
                    TransferCertificate.builder().tenantId(tid).studentName("Bruce Wayne").admissionNo("ADM-103")
                            .classLevel("10th").section("A").fatherName("Thomas Wayne").aadharNo("1111-2222-3333")
                            .tcNumber("TC-2026-003").issueDate(LocalDateTime.now().minusDays(3))
                            .pdfUrl("/tc/TC-2026-003.pdf").build());
            tcRepository.save(
                    TransferCertificate.builder().tenantId(tid).studentName("Diana Prince").admissionNo("ADM-201")
                            .classLevel("8th").section("C").fatherName("King Hippolyta").aadharNo("2222-3333-4444")
                            .tcNumber("TC-2026-004").issueDate(LocalDateTime.now().minusDays(1))
                            .pdfUrl("/tc/TC-2026-004.pdf").build());
            tcRepository.save(
                    TransferCertificate.builder().tenantId(tid).studentName("Peter Parker").admissionNo("ADM-105")
                            .classLevel("2nd").section("B").fatherName("Richard Parker").aadharNo("5555-6666-7777")
                            .tcNumber("TC-2026-005").issueDate(LocalDateTime.now().minusDays(7))
                            .pdfUrl("/tc/TC-2026-005.pdf").build());

            // ── JOB POSTINGS (5) ─────────────────────────────────────────
            JobPosting physJob = jobRepository.save(
                    JobPosting.builder().tenantId(tid).title("Senior Physics Faculty — IIT-JEE Wing")
                            .department("Competitive Coaching").qualification("M.Sc. / Ph.D. Physics or Engineering")
                            .experience("5+ years").description(
                            ...
                            .build());
            JobPosting mathJob = jobRepository.save(
                    JobPosting.builder().tenantId(tid).title("Primary Mathematics Teacher (Grades 1-5)")
                            .department("Academics").qualification("B.Sc. / B.Ed. Mathematics").experience("2-3 years")
                            .description(
            ...
            jobRepository.save(JobPosting.builder().tenantId(tid).title("School Counselor & Psychologist")
                    ...
                    .experience("3+ years").description(
            ...
            jobRepository.save(JobPosting.builder().tenantId(tid).title("Sports Coach — Cricket & Football")
                    ...
                    .experience("4+ years").description(
            ...
            applicationRepository.save(
                    JobApplication.builder().tenantId(tid).jobId(physJob.getId()).jobTitle(physJob.getTitle())
            ...
            applicationRepository.save(
                    JobApplication.builder().tenantId(tid).jobId(mathJob.getId()).jobTitle(mathJob.getTitle())
            ...
            // ── FEE ITEMS (5) ─────────────────────────────────────────────
            FeeItem tuition = feeItemRepository.save(
                    FeeItem.builder().tenantId(tid).name("Term 1 Tuition Fee").amount(15000.0).gradeLevel("All Grades")
                            .description(
                                    "Standard academic term tuition fee — covers all core subjects and teaching resources")
                            .build());
            FeeItem bus = feeItemRepository.save(
                    FeeItem.builder().tenantId(tid).name("Annual Bus Transportation Fee").amount(8000.0)
                            .gradeLevel("All Grades")
                            .description("Annual school bus pass covering all designated routes across Hyderabad")
                            .build());
            FeeItem lab = feeItemRepository.save(
                    FeeItem.builder().tenantId(tid).name("Science & Computer Lab Fee").amount(3000.0)
                            .gradeLevel("High School (G9-12)").description(
                                    "Annual lab usage fee covering consumables, equipment maintenance and practical sessions")
                            .build());
            FeeItem sports = feeItemRepository.save(
                    FeeItem.builder().tenantId(tid).name("Sports & Athletics Annual Fee").amount(2500.0)
                            .gradeLevel("All Grades").description(
                                    "Covers sports kit, coaching sessions, inter-school tournament registration and facilities")
                            .build());
            FeeItem jee = feeItemRepository.save(
                    FeeItem.builder().tenantId(tid).name("JEE/NEET Coaching Supplementary Fee").amount(25000.0)
                            .gradeLevel("High School (G11-12)").description(
            ...
            invoiceRepository.save(
                    StudentInvoice.builder().tenantId(tid).studentName("Harry Potter").admissionNo("ADM-901")
                            .gradeLevel("10th").section("A").fatherName("James Potter").aadharNo("1234-5678-9012")
                            .feeItemName(tuition.getName()).amount(tuition.getAmount()).status("PAID")
                            .dueDate(LocalDateTime.now().plusDays(30)).paymentDate(LocalDateTime.now().minusDays(2))
                            .build());
            invoiceRepository.save(
                    StudentInvoice.builder().tenantId(tid).studentName("Bruce Wayne").admissionNo("ADM-103")
                            .gradeLevel("10th").section("A").fatherName("Thomas Wayne").aadharNo("1111-2222-3333")
                            .feeItemName(tuition.getName()).amount(tuition.getAmount()).status("PAID")
                            .dueDate(LocalDateTime.now().plusDays(30)).paymentDate(LocalDateTime.now().minusDays(5))
                            .build());
            invoiceRepository.save(
                    StudentInvoice.builder().tenantId(tid).studentName("Ron Weasley").admissionNo("ADM-902")
                            .gradeLevel("12th").section("B").fatherName("Arthur Weasley").aadharNo("9876-5432-1098")
                            .feeItemName(tuition.getName()).amount(tuition.getAmount()).status("PENDING")
                            .dueDate(LocalDateTime.now().plusDays(10)).build());
            invoiceRepository.save(
                    StudentInvoice.builder().tenantId(tid).studentName("Hermione Granger").admissionNo("ADM-104")
                            .gradeLevel("12th").section("B").fatherName("Mr. Granger").aadharNo("4444-5555-6666")
                            .feeItemName(tuition.getName()).amount(tuition.getAmount()).status("PAID")
                            .dueDate(LocalDateTime.now().plusDays(30)).paymentDate(LocalDateTime.now().minusDays(8))
                            .build());
            invoiceRepository.save(
                    StudentInvoice.builder().tenantId(tid).studentName("Jane Smith").admissionNo("ADM-102")
                            .gradeLevel("2nd").section("B").fatherName("Robert Smith").aadharNo("9876-5432-1098")
                            .feeItemName(bus.getName()).amount(bus.getAmount()).status("PENDING")
                            .dueDate(LocalDateTime.now().plusDays(20)).build());
            invoiceRepository.save(
                    StudentInvoice.builder().tenantId(tid).studentName("Peter Parker").admissionNo("ADM-105")
                            .gradeLevel("2nd").section("B").fatherName("Richard Parker").aadharNo("5555-6666-7777")
                            .feeItemName(bus.getName()).amount(bus.getAmount()).status("PAID")
                            .dueDate(LocalDateTime.now().plusDays(30)).paymentDate(LocalDateTime.now().minusDays(1))
                            .build());
            invoiceRepository.save(
                    StudentInvoice.builder().tenantId(tid).studentName("Arjun Mehta").admissionNo("ADM-301")
                            .gradeLevel("12th").section("A").fatherName("Suresh Mehta").aadharNo("7777-8888-9999")
                            .feeItemName(jee.getName()).amount(jee.getAmount()).status("PAID")
                            .dueDate(LocalDateTime.now().plusDays(30)).paymentDate(LocalDateTime.now().minusDays(3))
                            .build());
            invoiceRepository.save(
                    StudentInvoice.builder().tenantId(tid).studentName("Priya Nambiar").admissionNo("ADM-302")
                            .gradeLevel("12th").section("A").fatherName("Rajan Nambiar").aadharNo("6666-7777-8888")
                            .feeItemName(jee.getName()).amount(jee.getAmount()).status("PENDING")
            ...
            gradeRepository.save(
                    StudentGrade.builder().tenantId(tid).studentName("Hermione Granger").admissionNo("ADM-104")
            ...
            gradeRepository.save(
                    StudentGrade.builder().tenantId(tid).studentName("Hermione Granger").admissionNo("ADM-104")
                            .classLevel("12th").section("B").fatherName("Mr. Granger").aadharNo("4444-5555-6666")
                            .subjectName("Chemistry").term("Term 1 Midterm").grade("A++")
            ...
            gradeRepository.save(StudentGrade.builder().tenantId(tid).studentName("Arjun Mehta").admissionNo("ADM-301")
                    ...
                    .build());
            gradeRepository.save(
                    StudentGrade.builder().tenantId(tid).studentName("Priya Nambiar").admissionNo("ADM-302")
                            .classLevel("12th").section("A").fatherName("Rajan Nambiar").aadharNo("6666-7777-8888")
                            .subjectName("Biology").term("Term 1 Midterm").grade("A+")
                            .remarks("Outstanding NEET-level biology answers. Strong conceptual clarity in genetics.")
                            .build());
            ...
            newsRepository.save(
                    SchoolNews.builder().tenantId(tid).title("NEET & JEE Topper Felicitation Ceremony — July 28")
                            .content(
            ...
            newsRepository.save(
                    SchoolNews.builder().tenantId(tid).title("New Robotics Lab Inaugurated — State-of-the-Art STEM Hub")
                            .content(
                            ...
                            .author("Principal Dr. Arthur Pendragon").publishedDate(LocalDateTime.now().minusDays(10))
                            .build());
            ...
            eventRepository.save(
                    SchoolEvent.builder().tenantId(tid).title("National Science & Robotics Olympiad — Campus Round")
                            .description(
            ...
            eventRepository.save(SchoolEvent.builder().tenantId(tid).title("Career Guidance & University Fair 2026")
                    ...
                    .build());
            eventRepository.save(
                    SchoolEvent.builder().tenantId(tid).title("Inter-School Debate & Model UN Championship")
                            .description(
                            ...
                            .eventDate(LocalDateTime.now().plusDays(42)).location("Main Auditorium, Madhapur Campus")
                            .build());
            ...
            admissionRepository.save(
                    AdmissionLead.builder().tenantId(tid).studentName("Robert Junior").gradeLevel("Kindergarten")
                            .parentName("Robert Senior").parentEmail("robert@email.com").parentPhone("+91 99001 12345")
                            .status("PENDING").message(
                                    "Inquiring about KG bus route schedules and school timing. Need admission for July 2026.")
                            .build());
            admissionRepository.save(
                    AdmissionLead.builder().tenantId(tid).studentName("Anika Sharma").gradeLevel("Grade 6")
                            .parentName("Deepak Sharma").parentEmail("deepak@email.com").parentPhone("+91 98765 11111")
                            .status("CONTACTED").message(
                                    "Looking for a school with strong STEM focus and sports for my daughter. Shifting from Pune.")
                            .build());
            admissionRepository.save(
                    AdmissionLead.builder().tenantId(tid).studentName("Rohan Gupta").gradeLevel("Grade 11 (Science)")
                            .parentName("Manoj Gupta").parentEmail("manoj@email.com").parentPhone("+91 99887 22222")
                            .status("PENDING").message(
                                    "Interested in IIT-JEE integrated program for Grade 11. Please share the detailed fee structure and hostel availability.")
                            .build());
            admissionRepository.save(
                    AdmissionLead.builder().tenantId(tid).studentName("Meera Iyer").gradeLevel("Grade 1")
                            .parentName("Suresh Iyer").parentEmail("suresh.iyer@email.com")
                            .parentPhone("+91 91234 33333").status("ENROLLED").message(
                                    "Would like to enroll my daughter for Grade 1. We visited the campus last week and loved the infrastructure.")
                            .build());
            admissionRepository.save(
                    AdmissionLead.builder().tenantId(tid).studentName("Vikrant Singh").gradeLevel("Grade 9")
                            .parentName("Rajinder Singh").parentEmail("rajinder@email.com")
                            .parentPhone("+91 93456 44444").status("PENDING").message(
            ...
            supportRepository.save(
                    SupportInquiry.builder().tenantId(tid).senderName("Diana Prince").senderEmail("diana@amazon.com")
                            .subject("Locker Key Card Replacement").message(
                                    "My daughter lost her gym locker key card. Who is the admin contact for issuing a replacement? She has PE class tomorrow.")
                            .status("RESOLVED")
                            .resolutionNotes("Directed to admin office. Replacement issued on July 28.").build());
            supportRepository.save(SupportInquiry.builder().tenantId(tid).senderName("Tony Stark")
                    .senderEmail("tony@starkindustries.com").subject("Fee Payment Receipt Not Received").message(
            ...
            supportRepository.save(SupportInquiry.builder().tenantId(tid).senderName("Priya Menon")
                    .senderEmail("priya.menon@email.com").subject("Request for Bonafide Certificate").message(
                    ...
                    .status("PENDING").build());
            supportRepository.save(
                    SupportInquiry.builder().tenantId(tid).senderName("Amit Verma").senderEmail("amit.verma@email.com")
                            .subject("School Bus Route Inquiry — Kondapur Area").message(
