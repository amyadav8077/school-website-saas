...
import com.schoolwebsite.backend.tenantsubscription.repository.*;
...
public class TenantService {
    ...
    public TenantResponse onboardTenant(TenantOnboardRequest request) {
        if (tenantRepository.existsBySubdomain(request.getSubdomain())) {
            throw AppException.conflict("Subdomain is already taken");
        }
        if (tenantRepository.existsByName(request.getName())) {
            throw AppException.conflict("School name is already registered");
        }
    ...
    public TenantResponse getTenantBySubdomain(String subdomain) {
        Tenant tenant = tenantRepository.findBySubdomain(subdomain)
                .orElseThrow(() -> AppException.notFound("Tenant not found with subdomain: " + subdomain));
    ...
    public TenantResponse updateCustomDomain(Long tenantId, String customDomain) {
        Tenant tenant = tenantRepository.findById(tenantId)
                .orElseThrow(() -> AppException.notFound("Tenant not found with id: " + tenantId));
    ...
    public TenantResponse cloneTenant(Long sourceTenantId, String newName, String newSubdomain) {
        ...
        Tenant sourceTenant = tenantRepository.findById(sourceTenantId)
                .orElseThrow(() -> AppException.notFound("Source Tenant not found with id: " + sourceTenantId));
