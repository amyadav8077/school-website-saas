...
public class EnrichmentActivityController {
    ...
    public ResponseEntity<ApiResponse<EnrichmentActivity>> createEnrichmentActivity(
            @PathVariable Long tenantId,
            @Valid @RequestBody EnrichmentActivity item) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(ApiResponse.ok("Enrichment activity created successfully", service.createActivity(tenantId, item)));
    }
