...
public class CareersServiceImpl implements CareersService {
    ...
    public void deleteJobPosting(Long id) {
        ...
        var existing = jobRepository.findById(id)
                .orElseThrow(() -> AppException.of(ErrorCode.JOB_POSTING_NOT_FOUND, id));
    ...
    public JobApplication submitApplication(Long tenantId, Long jobId, JobApplication application) {
        ...
        // The job must belong to the tenant named in the path, so an application
        // cannot be filed against another tenant's posting.
        if (!tenantId.equals(job.getTenantId())) {
            throw AppException.of(ErrorCode.JOB_POSTING_NOT_FOUND, jobId);
        }
    ...
    public JobApplication updateApplicationStatus(Long id, String status) {
        ...
        CurrentUser.assertTenantAccess(app.getTenantId());
