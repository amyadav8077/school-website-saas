...
public class AdmissionLeadController {
    ...
    public ResponseEntity<ApiResponse<AdmissionLeadResponse>> submitInquiry(
            @PathVariable Long tenantId,
            @Valid @RequestBody AdmissionLeadRequest request) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(ApiResponse.ok("Admission inquiry submitted successfully",
    ...
    public ResponseEntity<ApiResponse<AdmissionLeadResponse>> updateStatus(
